 ...
 * - assets/js/analytics.js: 分析功能前端脚本
 */
-->
<!DOCTYPE html>
<html lang="zh-CN">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <!-- 统计分析表 -->
                        <div id="analyticsTable" class="table-container mb-5" style="display: none;">
    ...
    <script>
                ...
                $.get('../api/index.php?route=score/teacher_subjects')
                    .done(function(response) {
                        if (response.success) {
                            ...
                            showAlert(response.error || '加载年级和科目失败');
                            reject(new Error(response.error || '加载失败'));
                        }
                    })
                    .fail(function(xhr) {
                        const error = '加载年级和科目失败：' + (xhr.responseJSON?.error || '未知错误');
                        showAlert(error);
        ...
        function loadSubjects(gradeId) {
            ...
            return new Promise((resolve, reject) => {
                const subjectSelect = $('#subjectFilter');
                subjectSelect.empty();
                subjectSelect.append('<option value="">选择科目</option>');
                ...
                .done(function(response) {
                    if (response.success && response.data) {
                        ...
                        if (uniqueSubjects.size === 0) {
                            subjectSelect.html('<option value="">该年级暂无可用科目</option>');
                        } else {
                        ...
                        const error = response.error || '加载科目失败';
                        showAlert(error);
                        reject(new Error(error));
                    }
                })
                .fail(function(xhr) {
                    const error = '加载科目失败：' + (xhr.responseJSON?.error || '未知错误');
                    showAlert(error);
                    reject(new Error(error));
                });
            });
        ...
        function loadClasses(gradeId) {
            return new Promise((resolve, reject) => {
                ...
                if (!subjectId) {
                    classSelect.append('<option value="">请先选择科目</option>');
                    resolve();
                    return;
                }
        ...
        function generateAnalyticsTable(analytics, schoolInfo) {
            ...
            */
            ...
            try {
                ...
                console.error('解析成绩分布数据失败:', error);
            }
        ...
        function loadScores() {
            ...
            if (!gradeId || !classId || !subjectId) {
                $('#scoreList').html('<div class="alert alert-info">请选择年级、科目和班级</div>');
                $('#analyticsTable').hide();
                return;
            }
        ...
        function generateScoreTable(data, subjectName) {
            ...
            data.forEach(student => {
                ...
                if (student.is_absent === '1' || student.is_absent === true) {
                    scoreText = '缺考';
                } else {
                    const baseScore = parseFloat(student.base_score || 0);
                    const extraScore = parseFloat(student.extra_score || 0);
        ...
        function sortScores(type) {
            ...
            if (type === 'number') {
                ...
                // 按成绩排序
                sortedData.sort((a, b) => {
                    const scoreA = a.is_absent ? -1 : (parseFloat(a.base_score || 0) + parseFloat(a.extra_score || 0));
                    const scoreB = b.is_absent ? -1 : (parseFloat(b.base_score || 0) + parseFloat(b.extra_score || 0));
                ...
                */
                if (response.success && response.data) {
                    ...
                    console.log('统计数据详情:', {
                        ...
                        schoolInfo: response.data.school_info
                    });
                    */
                    /* 记录关键统计指标
                    console.log('关键统计指标:', {
                        total_students: response.data.analytics.total_students,
                        attended_students: response.data.analytics.attended_students,
                        total_score: response.data.analytics.total_score,
                        avg_score: response.data.analytics.avg_score,
                        max_score: response.data.analytics.max_score,
                        min_score: response.data.analytics.min_score,
                        excellent_rate: response.data.analytics.excellent_rate,
                        pass_rate: response.data.analytics.pass_rate
                    });
                    */
                    // 记录分数分布数据
                    let distribution = {};
                    try {
                        if (typeof response.data.analytics.score_distribution === 'string') {
                            distribution = JSON.parse(response.data.analytics.score_distribution);
                        } else if (typeof response.data.analytics.score_distribution === 'object') {
                            distribution = response.data.analytics.score_distribution;
                        }
                    ...
                    // 记录生成的HTML
                    //console.log('生成的统计表HTML:', html);

                    return Promise.resolve();
                } else {
                    throw new Error('暂无统计数据，请等待成绩录入完成后自动生成');
                }
            })
            .catch(error => {
                // 记录错误信息
        ...
        $(document).ready(function() {
            ...
            $('#gradeFilter').change(function() {
                ...
                const subjectId = $('#subjectFilter').val();
                loadSubjects(gradeId).catch(console.error);
            });
            ...
            $('#subjectFilter').change(function() {
                ...
                if (!gradeId || !subjectId) {
                    $('#classFilter').empty().append('<option value="">请先选择年级和科目</option>');
                    $('#scoreList').empty();
                    $('#analyticsTable').hide();
                    return;
                }
            ...
            // 初始加载年级列表
            loadTeacherSubjects().catch(console.error);
        });
    </script>
</body>
</html> 